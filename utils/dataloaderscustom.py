...
from utils.general import (DATASETS_DIR, LOGGER, NUM_THREADS, TQDM_BAR_FORMAT, check_dataset, check_requirements,
                           check_yaml, clean_str, cv2, is_colab, is_kaggle, unzip_file, xyn2xy,
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self,
        ...
        for i, (label) in enumerate(zip(self.labels)):
            if include_class:
...
def verify_image_label(args):
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            nl = len(lb)
            if nl:
                assert lb.shape[1] == 14, f'labels require 14 columns, {lb.shape[1]} columns detected'
                ...
                ne += 1  # label empty
                lb = np.zeros((0, 14), dtype=np.float32)
        else:
            nm += 1  # label missing
            lb = np.zeros((0, 14), dtype=np.float32)
